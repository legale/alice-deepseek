...
- **Поиск в интернете через Google Custom Search API**: модель может самостоятельно решать, когда нужна актуальная информация из интернета, и выполнять поиск по запросу пользователя.
...
4. **Заполните `.env`**
    ...
    # для функции поиска в интернете:
    GOOGLE_API_KEY=ваш_ключ_Google_API
    GOOGLE_CX=ваш_идентификатор_поисковой_системы
    ```
    ...
    - `GOOGLE_API_KEY` и `GOOGLE_CX` — ключи для Google Custom Search API (необязательно, если не нужен поиск в интернете).
...
- **Поиск в интернете**: модель может самостоятельно решать, когда нужна актуальная информация, и выполнять поиск через Google Custom Search. Поиск работает автоматически при необходимости, включая случаи, когда выполнение занимает больше времени — в этом случае используется фоновая обработка.
