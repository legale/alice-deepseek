...
const SYSTEM_PROMPT = 'Ты голосовой ассистент, отвечай коротко по существу, без эмодзи. 
    ...
    Если пользователь просит найти что-то, 
    используй функцию search_internet. Важно: поиск выполняется только после подтверждения 
    пользователем. Сформулируй поисковый запрос максимально точно и информативно. Ответы поисковой системы надо
    обрабатывать и форматировать, чтобы было понятно и удобно для пользователя при восприятии на слух.';

// TECH_ERROR_MESSAGE определена в config.php
...
function normalize_content_parts($content): array
{
        ...
        if (empty($parts)) {
                $parts[] = [
                        'type' => 'text',
                        'text' => TECH_ERROR_MESSAGE . ' (пустой контент)'
                ];
        }
...
function build_display_text_from_parts(array $parts): string
{
        ...
        $text = trim(implode("\n", $texts));
        return $text !== '' ? $text : (TECH_ERROR_MESSAGE . ' (нет текста)');
}
